    ...
    <div class="w3-twothird">
     <?php  
      if(isset($_POST['updatelang'])){
         $name = $_POST['lname'];
         $level = $_POST['level'];
         $sql = "UPDATE languages SET langname = '$name', langlevel = '$level' WHERE id = $edit_id";
         if(mysqli_query($conn,$sql)){
          echo "<script>window.location='index.php';</script>";
         }
      }
      $sql= "SELECT * FROM languages WHERE id=$edit_id";
          $result = $conn->query($sql);
      ...
      <div class="w3-container w3-card w3-white w3-margin-bottom">
        <h2 class="w3-text-grey w3-padding-16"><i class="fa fa-suitcase fa-fw w3-margin-right w3-xxlarge w3-text-teal"></i>Edit Language</h2>
        <?php 
        if(isset($_GET['edit'])){
              $edit_id = $_GET['edit'];
         }
        ...
        <form class="w3-container w3-card-4" action="" method="Post">
          ...
          <input class="w3-input w3-border" name="lname" type="text" value="<?php echo $row['langname'];?>"></p>
          <p>      
          <label class="w3-text-blue"><b>Language Level</b></label>
          <input class="w3-input w3-border" name="level" type="number" value="<?php echo $row['langlevel'];?>"></p>
          <p>      
          <button class="w3-btn w3-blue" name="updatelang">Update</button></p>
        </form>
      </div>
